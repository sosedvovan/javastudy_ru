 ...
 *
 * Хочу отметить, что iText использует встроенный в spring класс
 * org.springframework.web.servlet.view.document.AbstractPdfView, но есть более современная
 * библиотека для работы с PDF — iTextPDF 5.5.1 (она не поддерживает библиотеки из этого примера).
 * Работа с iTextPDF 5.5.1 показана в похожей статье Spring Web Flow — создание PDF и Excel документов.
 * Нам понадобится создать два класса, в которых будет генерироваться соответствующий документ,
 * а так же контроллер, который будет обрабатывать запросы от клиента. Дополнительно был создан
 * класс-заглушка Cat чисто для формирования таблиц. Описывать подробно классы не буду,
 * т.к. все названия методов и переменных показывают их назначение (pdfHeader, addCell и т.п.).
 */
