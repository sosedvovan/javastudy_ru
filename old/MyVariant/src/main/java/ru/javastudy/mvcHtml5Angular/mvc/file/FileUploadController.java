...
/**
 * Java классы для работы с PDF, Excel, FileUpload
 * Created for JavaStudy.ru on 23.02.2016.
 * Загрузка файла
 * Для загрузки файла Spring MVC используется свой интерфейс MultipartResolver (мы будем использовать
 * реализующий его класс CommonsMultipartResolver). Работа с ним очень простая.
 * Всё что нужно сделать — указать маппинг, по которому контроллер перехватит запрос на закачку файла.
